...
const ManageBookings = () => {
  ...
  const handleUpdatePayment = async (billingId) => {
    try {
      await fetch(`${BASE_URL}/bookings/billings/${billingId}`, {
        ...
        body: JSON.stringify({ amount_paid: parseFloat(amountPaid), payment_status: paymentStatus }),
      });
  ...
  return (
    <div className="bg-white p-4 rounded-lg shadow-lg">
      <h2 className="text-2xl font-bold text-darkgreen mb-4">Manage Bookings</h2>

      {selectedBooking ? (
        <div>
          <button onClick={() => setSelectedBooking(null)} className="text-indigo-600 underline mb-2">
          ...
          {selectedBooking.Billings && selectedBooking.Billings.map((billing) => (
            <div key={billing.billing_id} className="mt-4 border-t pt-2">
              <p><strong>Amount Due:</strong> ${billing.amount_due.toFixed(2)}</p>
              <p><strong>Payment Status:</strong> {billing.payment_status}</p>
              <p><strong>Invoice Date:</strong> {new Date(billing.invoice_date).toLocaleString()}</p>
