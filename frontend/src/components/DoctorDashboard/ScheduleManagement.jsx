...
const ScheduleManagement = () => {
  ...
  const fetchAvailability = async () => {
    try {
      const response = await axios.get(
        `${BASE_URL}/doctors/${doctorId}/schedule`
      );
  ...
  const showEditPopup = async (dateKey) => {
    ...
    const scheduleNote = scheduleForDate
      ? scheduleForDate.note
      : "No note provided for this schedule.";
    ...
    MySwal.fire({
      ...
      html: `
        <div style="font-family: Arial, sans-serif; max-height: 80vh; overflow-y: auto;">
          <h3 style="color: #1B2E22; margin-bottom: 15px;">Appointments</h3>
          ${
            appointments.length > 0
              ? appointments
                  ...
                  <div style="display: flex; justify-content: space-between; margin-bottom: 10px;">
                    ...
                    <button data-id="${
                      appointment.id
                    }" class="cancel-appointment" style="color: white; background-color: red; border: none; padding: 5px 10px; cursor: pointer;">
                      Cancel
                    </button>
                  </div>`
                  )
          ...
          <h3 style="color: #1B2E22; margin-bottom: 15px;">Available Time Slots</h3>
          ${timeSlots
            .map(
              (slot) => `
                <div style="display: flex; justify-content: space-between; margin-bottom: 10px; align-items: center;">
                  <span style="flex-grow: 1;">${slot.start_time} - ${
                slot.end_time
              }</span>
                  ...
                  <button data-id="${
                    slot.id
                  }" class="toggle-availability" style="color: white; background-color: ${
                slot.is_available ? "green" : "gray"
            ...
            .join("")}
          <h3 style="color: #1B2E22; margin-top: 20px;">Add New Time Slot</h3>
        ...
        document
          ...
          .forEach((button) =>
            button.addEventListener("click", () =>
              handleCancelAppointment(button.getAttribute("data-id"))
            )
          );
        document
          .querySelectorAll(".toggle-availability")
          .forEach((button) =>
            button.addEventListener("click", () =>
              toggleAvailability(button.getAttribute("data-id"))
            )
          );
      },
      preConfirm: () => {
  ...
  const addNewTimeSlot = async (startTime, endTime, note, dateKey) => {
    if (!startTime || !endTime) {
      Swal.fire(
        "Please select both start and end times for the new time slot."
      );
      return;
    }
    ...
    try {
      ...
      if (scheduleForDate) {
        await axios.post(
          `${BASE_URL}/doctors/${doctorId}/schedule/${scheduleForDate.id}/timeslots`,
          {
            date: dateKey,
  ...
  const toggleAvailability = async (slotId) => {
    try {
      const response = await axios.patch(
        `${BASE_URL}/doctors/timeslots/${slotId}/toggle`
      );
      ...
      <Calendar
        ...
        tileContent={({ date }) => {
          ...
          if (
            ...
            availabilityInfo.timeSlots.length > 0
          ) {
            return (
              <div>{availabilityInfo.timeSlots.length} Slot(s) Available</div>
            );
          }
          return null;
        }}
