...
const getPatientInfo = async (req, res) => {
  try {
    const { searchTerm } = req.query; // Expecting search term in query (e.g. name, email, or condition)
    ...
    const patients = await User.findAll({
      where: {
        ...
        [Op.or]: [{ email: { [Op.like]: `%${searchTerm}%` } }],
      },
      include: [
        {
          model: UserProfile,
          ...
          where: searchTerm
            ...
            : undefined, // Include this condition only if searchTerm is provided
        },
        {
...
const updatePatientInfo = async (req, res) => {
  ...
  try {
    ...
    if (userProfile[0] > 0 || medicalRecordUpdated) {
      ...
      return res
        .status(400)
        .json({ message: "No changes made or patient not found." });
    }
  } catch (error) {
    console.error("Error updating patient info:", error);
    return res
      .status(500)
      .json({ message: "Failed to update patient information." });
  }
};
